...
const bannedWords = ["nigger","nigga","fuck","shit","bitch","cunt","asshole"];
...
module.exports = {
  ...
  async run(client, message, args) {
    ...
    if (text.length > 70) return message.reply("TTS limited to 70 characters.");
    ...
    state.queue.push({ text, user: message.author.username, channel });
...
async function playNext(guildId) {
  ...
  const { text, user } = state.queue.shift();

  let stream;
  try {
    stream = discordTTS.getVoiceStream(`${user} said ${text}`);
  } catch {
